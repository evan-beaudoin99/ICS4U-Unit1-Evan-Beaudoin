...
class Log {
  ...
  public static void main(final String[] args) {
    try {
      ...
      System.out.println(
          "Choose the length of the logs that you would like to move with a truck. ");
      System.out.println(
          "Enter the length of logs in meters between-> 0.25, 0.5, 1: ");
      ...
      if (length != 0.25 && length != 0.5 && length != 1) {
        throw new IllegalArgumentException(
            "Invalid length entered. Please enter 0.25, 0.5, or 1.");
      }

      double number_of_logs = LOGGING_TRUCK_LIMIT / (length * MASS_OF_LOGS);

      System.out.println("The truck can carry: " + number_of_logs + " logs at "
          + length + " meters");
